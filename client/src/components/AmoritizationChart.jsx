...
export const AmortizationChart = (amoritizationData) => {
	...
	const processData = (amortization) => {
		...
		const interestPayment = amortization.amortizationSchedule.map(
			(item) => ({
				x: new Date(item.year, parseInt(item.month % 12)),
				y: parseFloat(item.interestPayment),
			})
		);
		const principalPayment = amortization.amortizationSchedule.map(
			(item) => ({
				x: new Date(item.year, parseInt(item.month % 12)),
				y: parseFloat(item.principalPayment),
			})
		);
		const remainingPayment = amortization.amortizationSchedule.map(
			(item) => ({
				x: new Date(item.year, parseInt(item.month % 12)),
